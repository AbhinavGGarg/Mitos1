...
def cmd_fix(a):
    """One named copy, one named upstream fix. `backport` is this with discovery in front."""
    from .backport import transplant
    ...
    try:
        t = transplant(a.repo, a.sha, a.fix_path, a.target, a.target_path,
                       marker=a.marker, defined=defined, out_dir=a.out)
    except ValueError as e:
        print(red(str(e) + "; pass --marker")); sys.exit(1)
    except Exception as e:
        print(red(f"fetch failed: {e}")); sys.exit(1)

    print(f"  fix     : {cyan(a.repo + '@' + a.sha[:10])}  {dim(t.date[:10])}  {dim(t.message)}")
    print(f"  target  : {cyan(a.target)}  {dim(a.target_path)}  ({len(t.original_text.splitlines())} lines)")
    print(f"  marker  : {green(t.marker)}  {dim('(audited for executable placement)')}\n")

    print(bold(f"  transplanted {len(t.applied)} fix site(s):"))
    for s_ in t.applied:
        print(f"    {green('+')} {s_.first_code()[:78]}")
    if t.skipped:
        print(bold(f"\n  skipped {len(t.skipped)} (honest — not guessed):"))
        for s_ in t.skipped:
            print(f"    {yellow('·')} {dim(s_.first_code()[:44]):46} {dim(s_.reason)}")

    bad = [r for r in t.records if not r["ok"]]
    beh = t.behaviour
    print(bold("\n  verify:"))
    print(f"    original copy compiles : {green('yes') if t.compiles_before else red('no')}"
          + ("" if t.compiles_before else dim(f"   ({t.delta['before_errors']} pre-existing error(s)"
                                              f" — needs the host project's build flags)")))
    print(f"    patched copy  compiles : {green('yes') if t.compiles_after else red('no')}"
          + ("" if t.compiles_after else dim(f"   ({t.delta['after_errors']} error(s))")))
    if not (t.compiles_before and t.compiles_after):
        # A vendored file usually cannot build standalone. The answerable question is
        # whether the transplant introduced anything new.
        if t.delta["no_new_errors"]:
            print(f"    regression check       : {green('no new compiler errors introduced')}"
                  f"  {dim('← diagnostics identical before and after')}")
        else:
            print(f"    regression check       : {red(str(len(t.delta['introduced'])) + ' NEW error(s) introduced')}")
            for e in t.delta["introduced"][:3]:
                print(f"        {red('✗')} {dim(e[:88])}")
    ncode = len(t.records) - len(bad)
    tag = green(f"{ncode}/{len(t.records)} live & in the expected function") if t.placement_ok else \
        red(f"{len(bad)}/{len(t.records)} MISPLACED")
    ...
    out = t.out_dir
    print(bold("\n  ── result ──"))
    if t.verdict == "VERIFIED_BEHAVIOURAL":
        print(f"  {green('VERIFIED (behavioural)')} for {bold(a.target)} — {len(t.applied)} sites, every guard live & in the\n"
              f"  expected function, and the guard rejects an attack the original accepts.  {dim('→ ' + out)}")
    elif t.verdict == "VERIFIED_PLACEMENT_NO_REGRESSION":
        print(f"  {green('VERIFIED (placement, no regression)')} for {bold(a.target)} — {len(t.applied)} sites, every\n"
              f"  guard live and in the expected function. This copy does not build standalone (it needs the\n"
              f"  host project's flags), so the compile claim is the answerable one: {green('no new errors')}\n"
              f"  were introduced. {yellow('Not')} a claim that the patched copy builds in situ.  {dim('→ ' + out)}")
    elif t.verdict == "VERIFIED_PLACEMENT":
        extra = dim("no behavioural probe for this fix") if not beh.ran else red("behaviour probe did NOT fire — investigate")
        print(f"  {green('VERIFIED (placement)')} for {bold(a.target)} — {len(t.applied)} sites, every guard live & in the\n"
              f"  expected function (static).  {extra}  {dim('→ ' + out)}")
    else:
        print(f"  {red('NOT VERIFIED')} — {t.why_not}.")
...
def _slug(s: str) -> str:
    ...
    s = os.path.basename(s.rstrip("/")) if os.path.isabs(s) else s
    return "".join(ch if ch.isalnum() or ch in "._-" else "_" for ch in s).strip("_")


def cmd_backport(a):
    """One repository in, verified patches out. Discovery and the upstream coordinates are
    the machine's job; the only thing a person supplies is the repo they care about."""
    from .backport import CATALOG, BY_NAME, discover, discover_checkout, transplant
    print(bold("\nMitos") + dim("  —  backport  (find every stale vendored copy, fix each, verify each)\n"))

    libs = CATALOG
    if a.lib:
        if a.lib not in BY_NAME:
            print(red(f"unknown --lib {a.lib!r}; catalog: {', '.join(BY_NAME)}")); sys.exit(2)
        libs = (BY_NAME[a.lib],)
    if not (a.target or a.checkout):
        print(red("backport needs --target owner/repo or --checkout DIR")); sys.exit(2)

    label = a.target or os.path.abspath(a.checkout)
    # Artifacts default beside the process, never inside the tree being patched: anything
    # written there shows up as an untracked file and rides along into the pull request.
    out_root = a.out or os.path.join(os.getcwd(), "mitos-out")
    print(f"  target  : {cyan(label)}{dim('  (working tree)') if a.checkout else ''}")
    print(f"  catalog : {dim(', '.join(l.name for l in libs))}")
    # The probe builds a binary from the target's own source and runs it. That is fine in
    # the owner's CI and wrong for a repository someone else submitted.
    execute = a.execute and bool(a.checkout)
    if not execute:
        why = "not a checkout" if not a.checkout else "disabled"
        print(f"  probe   : {dim('behavioural execution off (' + why + ') — placement and compile still verified')}")
    print()

    try:
        if a.checkout:
            copies, excluded = discover_checkout(a.checkout, libs,
                                                 verbose=lambda m: print(dim(f"    {m}")))
        else:
            copies, excluded = discover(a.target, libs, verbose=lambda m: print(dim(f"    {m}")))
    except Exception as e:
        print(red(f"discovery failed: {e}")); sys.exit(1)

    stale = [c for c in copies if not c.patched]
    for lib_name, path, why in excluded:
        print(f"    {dim('·')} {dim(path)}  {dim(why)}")
    for c in copies:
        tag = green("carries the fix") if c.patched else red("MISSING the fix")
        print(f"  {'✓' if c.patched else '✗'} {c.lib.name:12} {c.path:44} {tag}  {dim(str(c.lines) + ' lines')}")
    if not copies:
        print(dim("\n  No implementation copies of catalog libraries found. This checks a small set of\n"
                  "  commonly copied C libraries; absence here is not evidence of absence.\n"))
        return 0
    if not stale:
        print(green("\n  Every copy found already carries its upstream fix. Nothing to backport.\n"))
        return 0

    print(bold(f"\n  {len(stale)} stale cop{'y' if len(stale) == 1 else 'ies'} — transplanting:\n"))
    results = []
    for c in stale:
        print(f"  {bold(c.lib.name)}  {dim(c.path)}")
        src, back = None, None
        if a.checkout:
            full = os.path.join(a.checkout, c.path)
            src = Path(full).read_text(encoding="utf8", errors="replace")
            back = full if a.in_place else None
        # One directory per copy, not per repo: a repo with two stale copies would
        # otherwise have the second overwrite the first's evidence.
        slug = "__".join(filter(None, [_slug(label), _slug(c.path)]))
        try:
            t = transplant(c.lib.upstream, c.lib.fix_sha, c.lib.fix_path, label, c.path,
                           defined=frozenset(c.lib.defines), write=not a.dry_run,
                           execute=execute, source_text=src, write_back=back,
                           out_dir=os.path.join(out_root, slug))
        except Exception as e:
            print(f"    {red('failed')}: {e}\n"); continue
        results.append(t)
        mark = green("✓") if t.verdict.startswith("VERIFIED") else red("✗")
        print(f"    {mark} {t.verdict}   {len(t.applied)} applied, {len(t.skipped)} skipped, "
              f"compiles {'yes' if t.compiles_before else 'no'}→"
              f"{'yes' if t.compiles_after else 'no'}")
        if not t.verdict.startswith("VERIFIED"):
            print(f"      {dim(t.why_not)}")
        else:
            if back:
                print(f"      {green('written in place')}  {dim(c.path)}")
            if not a.dry_run:
                print(f"      {dim('→ ' + t.out_dir)}")
        print()

    ok = [t for t in results if t.verdict.startswith("VERIFIED")]
    print(bold("  ── result ──"))
    print(f"  {green(str(len(ok)))} of {len(stale)} stale copies now carry the fix, each one "
          f"compiled and placement-audited.")
    if len(ok) < len(stale):
        print(dim(f"  {len(stale) - len(ok)} refused rather than guessed — see the reason under each."))
    print(dim("\n  A verified placement means every transplanted line is live code in the expected\n"
              "  function and the copy still compiles. It is not a claim that the shipped\n"
              "  application was exploitable, nor that the repo's own test suite was run.\n"))
...
def main(argv=None):
    ...
    pbp = sub.add_parser("backport", help="one repo in, verified patches out: find every stale vendored copy, fix and verify each")
    pbp.add_argument("--target", default=None, help="repository to backport into, e.g. revery-ui/revery")
    pbp.add_argument("--checkout", default=None, help="a working tree to scan instead of the API (use this in CI)")
    pbp.add_argument("--lib", default=None, help="restrict to one catalog library (default: all)")
    pbp.add_argument("--in-place", dest="in_place", action="store_true",
                     help="write verified patches back into the checkout, ready for a PR")
    pbp.add_argument("--no-execute", dest="execute", action="store_false",
                     help="never run the behavioural probe (it builds and runs the target's own code)")
    pbp.add_argument("--out", default=None,
                     help="where to write evidence (default ./mitos-out; keep it OUT of a checkout you intend to PR)")
