...
def build_once(log: Log, recipe: Recipe, wt: str, home: str, header_text: str, out_binary: str,
    ...
    # 2b. write any Mitos-shipped harness sources beside the header (same dirfd-relative
    #     O_NOFOLLOW path). getattr, not attribute access: tests inject minimal recipe stubs
    #     and a build must not depend on an optional field being present.
    for rel, data in (getattr(recipe, "harness_sources", None) or []):
